...
from evoworld.pipeline.pipeline_evoworld import convert_pano_to_mono
...
def main():
    ...
    if args.pretrain_mode:
        raise ValueError("--pretrain_mode is not supported by train_w_plucker_only.py.")
    if args.denoise_plucker:
        raise ValueError("--denoise_plucker is not supported by train_w_plucker_only.py.")
    if not args.add_plucker:
        raise ValueError("train_w_plucker_only.py requires --add_plucker.")
    if args.sampling_method != "empty_with_traj":
        raise ValueError(
            "train_w_plucker_only.py only supports --sampling_method empty_with_traj "
            "to avoid loading unused reprojection/memory images."
        )
    ...
    current_unet_in_channels = int(unet.conv_in.in_channels)
    expected_unet_in_channels = 14
    if current_unet_in_channels == 8:
        _replace_unet_conv_in_zero_init(
            unet,
            n_cond=1,
        ...
        current_unet_in_channels = int(unet.conv_in.in_channels)
    if current_unet_in_channels != expected_unet_in_channels:
        raise ValueError(
            "train_w_plucker_only.py expects a 14-channel UNet input "
            "(4 noisy + 4 first-frame + 6 Plucker). "
            f"Loaded UNet has {current_unet_in_channels} channels. "
            "Use a base 8-channel SVD UNet or a checkpoint already trained with "
            "the OnlyPluckerNoMemory 14-channel layout."
        )
    if int(unet.config.in_channels) != expected_unet_in_channels:
        unet.register_to_config(in_channels=expected_unet_in_channels)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                conditional_latents = conditional_latents.repeat(
                    1, num_frames, 1, 1, 1
                )
                ...
                if inp_noisy_latents.shape[2] != expected_unet_in_channels:
                    raise ValueError(
                        "Only-plucker UNet channel mismatch: "
                        f"input has {inp_noisy_latents.shape[2]} channels, "
                        f"UNet expects {expected_unet_in_channels}."
                    )

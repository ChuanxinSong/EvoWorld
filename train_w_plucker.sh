...
accelerate launch --config_file="config/${CONFIG_NAME}.yaml" \
    ...
    --width=$WIDTH \
    --height=$HEIGHT \
    --output_dir="$OUTPUT_ROOT/$DATASET_NAME-OnlyPlucker-$CONFIG_NAME-lr-$LR-step-$STEP-worldsize-$WORLD_SIZE-length-$NUM_FRAMES" \
    --logging_dir="$OUTPUT_ROOT/$DATASET_NAME-OnlyPlucker-$CONFIG_NAME-lr-$LR-step-$STEP-worldsize-$WORLD_SIZE-length-$NUM_FRAMES/logs" \
